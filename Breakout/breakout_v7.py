...
# Epsilon update time has been changed
...
model_path = '../models/breakoutDQN_v7.h5'
...
class QAgent:
    ...
    def create_model(self):
        ...
        model.add(keras.layers.Conv2D(16, kernel_size=[8, 8], strides=[4, 4], input_shape=[84, 84, 1],
                                      kernel_initializer=keras.initializers.VarianceScaling(scale=0.01,
                                                                                            distribution='normal'),
                                      activation='relu',
                                      padding='same', name='conv1'))
        model.add(keras.layers.Conv2D(32, kernel_size=[4, 4], strides=[2, 2], padding='same', name='conv2',
                                      kernel_initializer=keras.initializers.VarianceScaling(scale=0.01,
                                                                                            distribution='normal'),
                                      activation='relu'
                                      ))
    ...
    def train(self, eps):
        ...
        # Update Q_table
        Q_table[np.arange(self.batch_size), actions_] = rewards_ + (1 - dones_) * self.discount_rate * np.max(Q_next, axis=1)
...
if __name__ == '__main__':
    ...
    #agent.get_action = function(agent.get_action)
    #agent.store_memor y = function(agent.store_memory)
    action = None
    detailedSummary = []
    hit_rewards = list()
    for episode in range(episodes):
        ...
        current_reward = 0
        while True:
            if n_frame % 4 == 0: # states repeat 4 frames
